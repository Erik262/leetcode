...
class Solution:
    def isValidBST(self, root: Optional[TreeNode]) -> bool:
        ...
        queue = deque([(float("-infinity"), root, float("infinity"))])

        while queue:
            minVal, node, maxVal = queue.popleft()

            if not (minVal < node.val < maxVal):
                return False

            if node.left:
                queue.append((minVal, node.left, node.val))

            if node.right:
                queue.append((node.val, node.right, maxVal))
...
print(Solution().isValidBST(list_to_tree(root)))
